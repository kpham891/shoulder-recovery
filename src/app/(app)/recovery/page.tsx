...
export default function RecoveryPage() {
  ...
  const loadData = async () => {
    ...
    const [profileRes, logsRes, milestonesRes] = await Promise.all([
    ...
    if (completionsData) setCompletions(completionsData as unknown as WorkoutCompletion[]);
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold dark:text-white">Shoulder Recovery</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold dark:text-white">{rehabSessions}</span>
            </div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription className="text-xs">Log Streak</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-2">
              <Flame className="w-5 h-5 text-orange-500" />
              <span className="text-2xl font-bold dark:text-white">{streak}</span>
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">consecutive days</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Link href="/log">
          <Card className="hover:border-blue-400 dark:hover:border-blue-500 transition-colors cursor-pointer">
            <CardContent className="flex flex-col items-center gap-2 py-4">
              <Calendar className="w-6 h-6 text-blue-600 dark:text-blue-400" />
              <span className="text-sm font-medium dark:text-white">Daily Log</span>
            </CardContent>
          </Card>
        </Link>
        <Link href="/rehab">
          <Card className="hover:border-blue-400 dark:hover:border-blue-500 transition-colors cursor-pointer">
            <CardContent className="flex flex-col items-center gap-2 py-4">
              ...
              <span className="text-sm font-medium dark:text-white">Rehab</span>
            </CardContent>
          </Card>
        </Link>
        <Link href="/fitness">
          <Card className="hover:border-blue-400 dark:hover:border-blue-500 transition-colors cursor-pointer">
            <CardContent className="flex flex-col items-center gap-2 py-4">
              ...
              <span className="text-sm font-medium dark:text-white">Fitness</span>
            </CardContent>
          </Card>
        </Link>
        <Link href="/milestones">
          <Card className="hover:border-blue-400 dark:hover:border-blue-500 transition-colors cursor-pointer">
            <CardContent className="flex flex-col items-center gap-2 py-4">
              ...
              <span className="text-sm font-medium dark:text-white">Milestones</span>
            </CardContent>
          </Card>
        </Link>
      </div>
          ...
          <div className="space-y-3">
            ...
            {logs.length === 0 && (
              <p className="text-gray-500 dark:text-gray-400 text-center py-4">
                No logs yet. Start by logging how your shoulder feels today!
              </p>
            )}
          </div>
        </CardContent>
